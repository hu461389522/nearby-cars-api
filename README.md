...
一个基于 Go + Gin + GORM + Redis + MySQL 的后台管理 API 服务，支持数据增删查、缓存同步、AI 智能客服，可作为内部工具、AI 应用、管理后台等场景的后端底座。
...
浏览器直接打开，输入问题即可与 AI 对话：

👉 **http://139.199.18.67:8080/**
...
- **Service 层**：实现核心业务逻辑（MySQL + Redis 同步、AI 调用）
...
│   └── car.go              # 数据模型定义
├── service/
│   ├── car_service.go      # 车辆业务逻辑
│   └── ai_service.go       # AI 智能客服逻辑
├── controller/
...
- 数据查询 API：http://139.199.18.67:8080/nearby?lng=116.42&lat=39.90
...
- Redis 缓存加速
- **对接 DeepSeek 大模型 API，实现 AI 智能客服**
- 服务启动时自动加载缓存
